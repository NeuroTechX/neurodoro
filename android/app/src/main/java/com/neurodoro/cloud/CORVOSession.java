...
/** A Java Object that can be converted to an LSL-ish data format four our DB */
public class CORVOSession {
  ...
  public void addSample(double[] data, int timestamp) {
    samples.add(new StreamDataChunk(data, timestamp));
  }
  ...
  private class StreamDataChunk {
    ...
    public StreamDataChunk(double[] data, int timestamp) {
      this.data = data;
  ...
  public static void main(String[] args) {
    ...
    for (int i = 0; i < testSamples.length; i++) {
      try {
        sesh.addSample(testSamples[i], (int) System.currentTimeMillis() - (int) startTime);
        Thread.sleep(25);
      } catch (Exception e) {
